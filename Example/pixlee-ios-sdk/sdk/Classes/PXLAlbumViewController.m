...
@property (nonatomic, strong) UIButton *gridButton, *listButton, *uploadButton;
...
- (void)setupDisplayButtons {
    const CGFloat kUploadButtonTopMargin = 20;
    const CGFloat kButtonTopMargin = 80;
    const CGFloat kButtonHeight = 44;
    
    self.gridButton = [self displayButtonWithImageName:@"grid.png" selector:@selector(displayButtonPressed:)];
    [self.view addSubview:self.gridButton];
    self.listButton = [self displayButtonWithImageName:@"column.png" selector:@selector(displayButtonPressed:)];
    [self.view addSubview:self.listButton];
    self.uploadButton = [self displayButtonWithImageName:@"upload.png" selector:@selector(uploadButtonPressed:)];
    ...
    [self.uploadButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view.mas_top).with.offset(kUploadButtonTopMargin);
        make.left.equalTo(self.view.mas_centerX);
        make.right.equalTo(self.view.mas_right);
        make.height.equalTo(@(kButtonHeight));
    }];
...
//    [self.album.photos insertObject:imageData atIndex:0];
